(function(window) {
	...
	var leftArrowClicked = function(e) {
		...
		if (parseInt($boxes.css("left")) > -parseInt($boxes.css("width")) + 562) {
	...
	var rightArrowClicked = function(e) {
		...
		if (parseInt($boxes.css("left")) < 0) {
			TweenMax.to($boxes, 0.8, { left : "+=552", ease : Expo.easeOut } );
		}
		
		// animate back to the end
		else {
			TweenMax.to($boxes, 1, { left : -parseInt($boxes.css("width")) + 562, ease : Expo.easeOut } );
		}
	}
